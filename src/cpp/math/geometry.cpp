...
bool Geometry::is_triangle_in_cube(const P3D& centre, float half_edge, const P3D& a, const P3D& b, const P3D& c)
{
    ...
    if (a.x < ck and b.x < ck and c.x < ck)
        return false;
    ck = centre.x + half_edge;
    if (a.x > ck and b.x > ck and c.x > ck)
        return false;
    ...
    if (a.y < ck and b.y < ck and c.y < ck)
        return false;
    ck = centre.y + half_edge;
    if (a.y > ck and b.y > ck and c.y > ck)
        return false;
    ...
    if (a.z < ck and b.z < ck and c.z < ck)
        return false;
    ck = centre.z + half_edge;
    if (a.z > ck and b.z > ck and c.z > ck)
        return false;
...
{
    // FIXIT-L
    // Should be a define or const for srqr
...
bool Geometry::is_cube_visible(const P3D& centre, float half_edge) const
{
    ...
    for (int i = 0; i < 6; ++i)
    {
        ...
        if (xm + ym + zm + piramid_vis[i][3] > 0)
            continue;

        xp = piramid_vis[i][0] * (centre.x + half_edge);
        if (xp + ym + zm + piramid_vis[i][3] > 0)
            continue;

        yp = piramid_vis[i][1] * (centre.y + half_edge);
        if (xm + yp + zm + piramid_vis[i][3] > 0)
            continue;
        if (xp + yp + zm + piramid_vis[i][3] > 0)
            continue;

        zp = piramid_vis[i][2] * (centre.z + half_edge);
        if (xm + ym + zp + piramid_vis[i][3] > 0)
            continue;
        if (xp + ym + zp + piramid_vis[i][3] > 0)
            continue;
        if (xm + yp + zp + piramid_vis[i][3] > 0)
            continue;
        if (xp + yp + zp + piramid_vis[i][3] > 0)
            continue;
...
int Geometry::is_box_visible(const P3D& origin, const P3D& sides) const
{
    ...
    // FIXIT-L
    // Should be a define or const for all literal indexes
    for (int i = 0; i < 6; ++i)
    {
        int c = 0;
...
void Geometry::calc_mirror_matrix(const P3D& plane_point, const V3D& plane_normal, float* reflection_matrix)
{
    ...
    float d = plane_point.x * plane_normal.dir.x
            + plane_point.y * plane_normal.dir.y
            + plane_point.z * plane_normal.dir.z;
...
void Geometry::set_piramid_vis(const float* proj, const float* modl)
{
    ...
    t = sqrt(piramid_vis[0][0] * piramid_vis[0][0]
            + piramid_vis[0][1] * piramid_vis[0][1]
            + piramid_vis[0][2] * piramid_vis[0][2]);
    piramid_vis[0][0] /= t;
    piramid_vis[0][1] /= t;
    piramid_vis[0][2] /= t;
    piramid_vis[0][3] /= t;
    ...
    t = sqrt(piramid_vis[1][0] * piramid_vis[1][0]
            + piramid_vis[1][1] * piramid_vis[1][1]
            + piramid_vis[1][2] * piramid_vis[1][2]);
    piramid_vis[1][0] /= t;
    piramid_vis[1][1] /= t;
    piramid_vis[1][2] /= t;
    piramid_vis[1][3] /= t;
    ...
    t = sqrt(piramid_vis[2][0] * piramid_vis[2][0]
            + piramid_vis[2][1] * piramid_vis[2][1]
            + piramid_vis[2][2] * piramid_vis[2][2]);
    piramid_vis[2][0] /= t;
    piramid_vis[2][1] /= t;
    piramid_vis[2][2] /= t;
    piramid_vis[2][3] /= t;
    ...
    t = sqrt(piramid_vis[3][0] * piramid_vis[3][0]
            + piramid_vis[3][1] * piramid_vis[3][1]
            + piramid_vis[3][2] * piramid_vis[3][2]);
    piramid_vis[3][0] /= t;
    piramid_vis[3][1] /= t;
    piramid_vis[3][2] /= t;
    piramid_vis[3][3] /= t;
    ...
    t = sqrt(piramid_vis[4][0] * piramid_vis[4][0]
            + piramid_vis[4][1] * piramid_vis[4][1]
            + piramid_vis[4][2] * piramid_vis[4][2]);
    piramid_vis[4][0] /= t;
    piramid_vis[4][1] /= t;
    piramid_vis[4][2] /= t;
    piramid_vis[4][3] /= t;
    ...
    t = sqrt(piramid_vis[5][0] * piramid_vis[5][0]
            + piramid_vis[5][1] * piramid_vis[5][1]
            + piramid_vis[5][2] * piramid_vis[5][2]);
    piramid_vis[5][0] /= t;
    piramid_vis[5][1] /= t;
    piramid_vis[5][2] /= t;
    piramid_vis[5][3] /= t;
